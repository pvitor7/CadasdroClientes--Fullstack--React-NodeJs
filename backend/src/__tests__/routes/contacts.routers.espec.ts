...
describe("Teste de listagem de contatos", () => {
    ...
    test("Test listar contatos de um cliente", async () => {
        ...
        const response = await request(app).get(`/user/${newClientId}/contact`);
...
describe("Teste de exclusão de contatos", () => {
    ...
    test("Testar excluir contatos para cliente com id inválido", async () => {
        const response = await request(app).delete(`/user/${invalidId}/contact/1`);
        expect(response.status).toBe(400);
        expect(response.body).toHaveProperty("message")
        expect(response.body.message).toBe("Usuário não encontrado")
    })
    ...
    test("Testar excluir contato", async () => {
        ...
        await ContactCreateService(clientId, contact1Data.type, contact1Data.email, contact1Data.phone);
        await ContactCreateService(clientId, contact2Data.type, contact2Data.email, contact2Data.phone);
        await ContactCreateService(clientId, contact3Data.type, contact3Data.email, contact3Data.phone);
        const contact = await ListContactsClient(clientId)
        const response = await request(app).delete(`/user/${clientId}/contact/${contact.contacts[0].id}}`);
